...
# Cluster configuration: 5 clusters per cell × 3 integrins per cluster
CLUSTERS_PER_CELL     = 10
INTEGRINS_PER_CLUSTER = 4

# For reference: total integrins per cell
...
VOR_VERTICES_PATH = "vertices_young37.out" # change as needed
VOR_EDGES_PATH    = "nodes_to_edges_young37.out"  # change as needed
VORONOI_SCALE     = 0.5
CENTER_VORONOI    = True
FIBER_LIMIT       = 1000
...
def log_integrin_force(step, time_s, integ_id, cluster_id, fiber_idx, node_idx, e, F_sim, F_phys):
    """
    Log per-integrin force, including cluster_id so we can aggregate forces per cluster.
    cluster_id can be None; we store -1 in that case.
    """
    cid_val = -1 if cluster_id is None else int(cluster_id)
...
class Integrin:
    def __init__(self, view, id, base_position, length=0.03*0.5, radius=0.0025,
        ...
        self.cluster_id = cluster_id  # which cluster on this cell (1..CLUSTERS_PER_CELL), or None
    ...
    def switch_to_active(self, attach_pt=None):
        ...
        if attach_pt is not None:
            pivot = self._pivot_point()
            attach_pt = np.array(attach_pt, dtype=np.float64)
            # purely geometric length check here; gating is done in the manager
...
class IntegrinManager:
    ...
    def populate_integrins_on_sphere(self, N, center, radius):
        """(Old) uniform distribution, still available if you want it."""
    ...
    def populate_integrin_clusters_on_cell(
        ...
        """
        Place integrins in clusters on a spherical cell surface.
        Each cell gets `clusters_per_cell` clusters, each with
        `integrins_per_cluster` integrins packed close together.
        """
        center = np.array(center, dtype=np.float64)

        # First choose cluster centers on sphere
        cluster_centers = fibonacci_sphere_points(clusters_per_cell, radius, center)

        for cid, c_pt in enumerate(cluster_centers, start=1):
            # Normal from cell center through the cluster center
            n = _unit(c_pt - center)
            # Tangent basis around n for spreading integrins in a small patch
            ref = np.array([0.0, 0.0, 1.0]) if abs(n[2]) < 0.9 else np.array([1.0, 0.0, 0.0])
            t1 = _unit(np.cross(ref, n))
            t2 = _unit(np.cross(n, t1))

            # Small "cluster radius" on the surface
            cluster_radius = 0.2 * radius  # tweak for tighter/looser clusters
            ...
            for k in range(integrins_per_cluster):
                ...
                # Base position: around the nominal cluster point on the sphere
        ...
        for integ in self.integrins.values():
            ...
            if best is not None and min_dist < reach:
                ...
                if attach_idx is not None:
                    ...
                    # geometric front/back gate using integrin normal:
                    # only bind if attachment is in front of the integrin (dot >= 0)
                    if np.dot(rel, integ.n) >= 0.0:
                        L_bind_top = float(np.linalg.norm(rel))
...
view.setWindowTitle('Integrin Springs (Compression + Preload at Bind) + Catch–Slip + Fiber–Fiber Links (Voronoi, free-end fibers, no L0 shrink, clustered integrins)')
...
def add_random_cell_with_integrins(max_attempts=50, min_gap_factor=1.05):
    ...
    for attempt in range(max_attempts):
        ...
        if ok:
            ...
            manager.populate_integrin_clusters_on_cell(
                center=center,
                radius=CELL_RADIUS,
            )
...
def frame_2():
    ...
    with open(INTEGRIN_FORCE_CSV, "w", newline="") as fh:
        w = csv.writer(fh)
        w.writerow(["step","time_s","integrin","cluster_id","fiber_idx","node_idx",
...
def simulation_step():
    ...
    try:
        ...
        for integ in active:
            ...
            log_integrin_force(current_step, sim_time, integ.id, integ.cluster_id, fib_idx, idx,
                               e, F_sim, F_phys)

            # --- force-based rupture cap (based on magnitude) ---
            F_CAP = 260e-12  # 260 pN rupture threshold
        ...
        if current_step >= TOTAL_STEPS:
            ...
            plot_cluster_force_over_time()
            plot_fiber_displacement_over_time()
            plot_normalized_fiber_displacement_over_time()
...
def plot_integrin_forces(csv_path="integrin_forces.csv", out_path="integrin_force_over_time.png"):
    ...
    if not os.path.exists(csv_path):
        print(f"[WARN] {csv_path} not found; skipping integrin force plot.")
        return

    df = pd.read_csv(csv_path)
    if df.empty:
        print("[WARN] integrin_forces.csv is empty; skipping integrin force plot.")
        return
...
def plot_cluster_force_over_time(
    csv_path="integrin_forces.csv",
    out_path="cluster_force_over_time.png"
):
    """
    Plot total force per integrin cluster vs time.
    Each line is one cluster, summing F_phys over all integrins in that cluster.
    """
    if not os.path.exists(csv_path):
        print(f"[WARN] {csv_path} not found; skipping cluster force plot.")
        return

    df = pd.read_csv(csv_path)
    if df.empty or "cluster_id" not in df.columns:
        print("[WARN] No cluster_id information in integrin force CSV; skipping cluster force plot.")
        return
    ...
    # Drop non-clustered integrins (cluster_id == -1)
    df = df[df["cluster_id"] > 0].copy()
    if df.empty:
        print("[WARN] No clustered integrins found; skipping cluster force plot.")
        return

    # Sum F_phys per (cluster_id, time_s)
    cl = (
        df.groupby(["cluster_id", "time_s"])["F_phys"]
        .sum()
        .reset_index()
        .sort_values(["cluster_id", "time_s"])
    )

    plt.figure(figsize=(12, 8))
    for cid, g in cl.groupby("cluster_id"):
        plt.plot(g["time_s"], g["F_phys"] * 1e12, label=f"Cluster {cid}")  # N → pN

    plt.xlabel("Simulation Time (s)")
    plt.ylabel("Total Cluster Force (pN)")
    plt.title("Cluster Force vs Time (sum of integrin forces per cluster)")
    plt.legend(title="Cluster ID", bbox_to_anchor=(1.05, 1.0), loc="upper left", fontsize="small")
    plt.tight_layout()
    plt.grid(True)
    plt.savefig(out_path, dpi=200)
    plt.close()

    print(f"[✓] Saved cluster force plot to {out_path}")
...
def plot_fiber_displacement_over_time(
    ...
    """
    if not os.path.exists(deform_csv):
        print(f"[WARN] {deform_csv} not found; skipping fiber displacement plot.")
        return
...
def plot_normalized_fiber_displacement_over_time(
    deform_csv="fiber_deformation3.csv",
    out_path="fiber_displacement_over_time_top10_normalized.png",
    top_n=10
):
    """
    Like plot_fiber_displacement_over_time, but each fiber's mean displacement
    time course is normalized by its own maximum displacement across steps.
    """
    if not os.path.exists(deform_csv):
        print(f"[WARN] {deform_csv} not found; skipping normalized displacement plot.")
        return
    ...
    common = b.index.intersection(a.index)
    if len(common) == 0:
        print("[WARN] No matching BEFORE/AFTER entries found for normalized displacement.")
        return
    ...
    if fiber_disp.empty:
        print("[WARN] No displacement data available for normalized fibers.")
        return

    # Rank fibers by total displacement
    fiber_total = (
        fiber_disp.groupby("Fiber")["displacement"]
        .sum()
        .sort_values(ascending=False)
    ...
    top_fibers = list(fiber_total.head(top_n).index)
    print(f"[INFO] Top {len(top_fibers)} fibers by cumulative displacement (for normalized plot):", top_fibers)

    fiber_disp_top = fiber_disp[fiber_disp["Fiber"].isin(top_fibers)].copy()
    if fiber_disp_top.empty:
        print("[WARN] No data for top fibers in normalized displacement.")
        return

    # Normalize per fiber by its max displacement across steps
    max_per_fiber = fiber_disp_top.groupby("Fiber")["displacement"].transform(lambda x: x.max())
    fiber_disp_top["disp_norm"] = fiber_disp_top["displacement"] / max_per_fiber.replace(0, np.nan)

    plt.figure(figsize=(12, 8))
    for fiber_id, g in fiber_disp_top.groupby("Fiber"):
        g = g.sort_values("step")
        plt.plot(g["step"], g["disp_norm"], label=f"Fiber {fiber_id}")

    plt.xlabel("Simulation Step")
    plt.ylabel("Normalized Mean Fiber Displacement")
    plt.title(f"Normalized Fiber Displacement vs Time (Top {len(top_fibers)} Most Deformed Fibers)")
    plt.legend(title="Fiber ID", bbox_to_anchor=(1.05, 1.0), loc="upper left", fontsize="small")
    plt.ylim(0, 1.05)
    plt.tight_layout()
    plt.grid(True)
    ...
    print(f"[✓] Saved normalized fiber displacement plot to {out_path}")
...
def plot_force_vs_displacement(
    ...
    """
    if not os.path.exists(deform_csv):
        print(f"[WARN] {deform_csv} not found; skipping force–displacement plot.")
        return
    if not os.path.exists(bond_csv):
        print(f"[WARN] {bond_csv} not found; skipping force–displacement plot.")
        return
